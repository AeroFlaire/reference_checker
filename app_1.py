...
def check_references():
    ...
    try:
        ...
        # --- CHANGED: Create lists for the 3 categories ---
        results_verified = []
        results_flawed = []
        results_not_found = []
        ...
        for ref_string in reference_strings:
            ...
            if "error" in parsed_data or (not parsed_title and not parsed_author):
                ...
                # --- WATERFALL LOGIC (This is our working order) ---
                ...
                # Attempt 2: "Dumb" Search (General `search` with title)
                if not oa_results and parsed_title:
                    print(f"Strict search failed. Trying fallback (General Search) for '{parsed_title}'...")
            ...
            # --- VERIFICATION STEP (NEW 3-CATEGORY LOGIC) ---
            
            # Default status
            status = "NOT_FOUND" 
            best_flawed_match = None
            verified_match = None
            
            if oa_results:
                # Loop through top 5 results
                for match_candidate in oa_results[:5]:
                    ...
                    # If we are here, the title is a match.
                    # It is, at minimum, a "FLAWED_REFERENCE"
                    print(f"Python Title Check: PASS (Score: {title_score}) for '{found_title}'")
                    status = "FLAWED_REFERENCE"
                    best_flawed_match = match_candidate # Store this
                    ...
                    if year_verdict == "PASS":
                        ...
                        status = "VERIFIED"
                        verified_match = match_candidate
                        break # Stop looping, we found our paper
